  ...
  run: async (payload: any, { ctx }) => {
    ...
    for (const remote of allNewAgencies) {
      ...
      for (const local of existingAgencies) {
        try {
          // Normalize names for comparison (strip VA, Inc, etc. inside Zig if possible, or here)
          const score = parseFloat(
            execSync(`./zig-engine/match "${remote.name}" "${local.name}"`, { timeout: 1000 }).toString().trim()
          );
          
          if (!isNaN(score) && score > 0.85) {
            isDuplicate = true;
            // Update lastSync and potentially merge metadata
