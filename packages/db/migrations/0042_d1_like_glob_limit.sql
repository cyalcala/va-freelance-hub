-- EX-02: D1 LIKE/GLOB patterns are capped at 50 bytes.
-- The 92-byte ISO-8601 GLOB in 0039-0041 aborts the first source_registry
-- insert with LIKE or GLOB pattern too complex. Canonical UTC remains
-- length=24, hour range, and strftime round-trip. Activates no source.
-- CASE expressions stay parenthesized for D1's remote parser (workers-sdk#4727).

-- stripped 1 overlong GLOB clause(s) from source_registry_policy_expiry_is_canonical_utc_insert
DROP TRIGGER IF EXISTS source_registry_policy_expiry_is_canonical_utc_insert;
CREATE TRIGGER source_registry_policy_expiry_is_canonical_utc_insert
...
-- stripped 1 overlong GLOB clause(s) from source_registry_policy_expiry_is_canonical_utc_update
DROP TRIGGER IF EXISTS source_registry_policy_expiry_is_canonical_utc_update;
CREATE TRIGGER source_registry_policy_expiry_is_canonical_utc_update
...
-- stripped 5 overlong GLOB clause(s) from source_admission_evidence_validate_insert
DROP TRIGGER IF EXISTS source_admission_evidence_validate_insert;
...
BEGIN
  ...
  WHERE NOT (length(NEW.captured_at) = 24 AND substr(NEW.captured_at,12,2) BETWEEN '00' AND '23' AND strftime('%Y-%m-%dT%H:%M:%fZ',NEW.captured_at) IS NEW.captured_at) OR NOT (length(NEW.expires_at) = 24 AND substr(NEW.expires_at,12,2) BETWEEN '00' AND '23' AND strftime('%Y-%m-%dT%H:%M:%fZ',NEW.expires_at) IS NEW.expires_at)
  ...
  WHERE json_type(NEW.packet_json,'$.primaryEvidence') IS NOT 'array'
    ...
    OR EXISTS(SELECT 1 FROM json_each(NEW.packet_json,'$.primaryEvidence') WHERE
      ...
      OR NOT (length(json_extract(value,'$.capturedAt')) = 24 AND substr(json_extract(value,'$.capturedAt'),12,2) BETWEEN '00' AND '23' AND strftime('%Y-%m-%dT%H:%M:%fZ',json_extract(value,'$.capturedAt')) IS json_extract(value,'$.capturedAt'))
  ...
  WHERE NOT EXISTS(
    ...
    WHERE s.source_id=NEW.source_id AND p.id=NEW.provider_id
      ...
      AND (length(s.policy_expiry) = 24 AND substr(s.policy_expiry,12,2) BETWEEN '00' AND '23' AND strftime('%Y-%m-%dT%H:%M:%fZ',s.policy_expiry) IS s.policy_expiry) AND julianday(s.policy_expiry)>=julianday(NEW.expires_at)
      AND (length(p.evidence_captured_at) = 24 AND substr(p.evidence_captured_at,12,2) BETWEEN '00' AND '23' AND strftime('%Y-%m-%dT%H:%M:%fZ',p.evidence_captured_at) IS p.evidence_captured_at) AND julianday(p.evidence_captured_at)<=julianday(NEW.captured_at)
...
-- stripped 1 overlong GLOB clause(s) from source_shadow_observations_admission_insert
...
-- stripped 4 overlong GLOB clause(s) from source_transition_events_validate_insert
...
-- stripped 1 overlong GLOB clause(s) from source_publication_ledger_validate_insert
DROP TRIGGER IF EXISTS source_publication_ledger_validate_insert;
