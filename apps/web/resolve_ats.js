...
async function run() {
  const pendingAgencies = agencies.filter(a => !state[a.id]);
  console.log(`Total agencies: ${agencies.length}. Pending: ${pendingAgencies.length}.`);

  if (pendingAgencies.length === 0) {
    console.log("All agencies already processed.");
    return;
  }
