...
export const SignalCard = ({ signal }: { signal: any }) => {
      ...
      <div class="flex items-center gap-4 sm:gap-6 w-full overflow-hidden">
        ...
        <div class="min-w-0 flex-1">
          ...
          <div class="flex items-center gap-2">
            <span class="text-[10px] text-blueberry-800/60 font-bold uppercase tracking-widest truncate">{signal.company}</span>
            {signal.tier < 2 && <span class="text-[10px]">🔥</span>}
          </div>
        </div>
      </div>
      <div class="flex items-center justify-between sm:justify-end gap-4 text-blueberry-800/20">
         <span class="text-[9px] font-black uppercase tracking-[0.2em]">{signal.sourcePlatform}</span>
