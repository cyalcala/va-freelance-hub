...
| D1 Registry Canaries | 0 sources currently in canary |
| D1 Registry Shadows | 15 sources in `operational_state = 'shadow'` (Workable x7, Greenhouse x6, Recruitee x1, Breezy x1) |
...
- **Canary -> Active Graduation of 5 Philippine VA Agencies Live**:
  ...
  - Zero sources currently in canary; 15 in shadow; 14 in candidate queue (`needs_review`).
  - Quarantined: `teamtailor:career.teamtailor.com` (HTTP 404).
- **Stratified Audit of 7d Unclear Losses Completed**:
  - `docs/audits/STRATIFIED_UNCLEAR_LOSS_AUDIT_2026-09-25.md` completed.
  - Identified 22 onsite Sourcefit jobs violating remote policy (to be deactivated) and 85 genuine remote jobs (33 Sourcefit + 52 20Four7VA) held unclear due to `recoverGateEligiblePending` dropping gate tiers.
