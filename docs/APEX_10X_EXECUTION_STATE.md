...
| Current 7d Qualified Baseline | 202 jobs / 7 days = 28.86 jobs/day (first-stored proxy, snapshot `2026-09-25T10-33-43-612Z.json`) |
| Clocks | Primary Cloudflare Worker `freshness-cron` (10m scrape + hourly shadow dispatch :20Z); secondary Hunter in standby; APEX economics cron daily |
...
- **Shadow -> Canary Promotion of GitLab & Grafana Labs Executed; Workable Empirical Audit**:
  ...
  - Gated feed reactivation in `scrape.ts` to require verified/likely PH eligibility.
  - Verified remote D1 truth: 854 active opportunities, 100% constitutionally eligible (745 `eligible_likely`, 109 `eligible_verified`), 0 unclear active.
  - Snapshot `2026-09-25T10-33-43-612Z.json` records 202 net-new in 7d = **28.86 jobs/day** (up from 17.14/day). All unclear loss investigation items resolved cleanly.
- **Shadow -> Canary Promotion of 3 Mature Defect-Free Sources Executed**:
  - `greenhouse:ghost` (cap 2), `greenhouse:nearform` (cap 2), `breezy:time-etc` (cap 1) formally promoted from `shadow` to `canary` in remote production D1 via `scripts/graduation/promote-proven-shadow-canary.ts`.
  - All 3 sources verified with 12.00–13.44 day clean spans, 13–15 qualifying observation dates, 0 anomalies, valid leases through 2027, and `public_minimal_metadata_canary` authority.
  - Jev 1.13 decision trace confirmed `Variant_A` (promote to canary, confidence 0.73).
  - Canary publication clamp verified live for Nearform in tick `09:30:10Z` (proposed: 1, published: 1, mode: capped).
  - Registry truth: 5 active / 3 canary / 12 shadow / 14 candidate / 1 quarantined = 35 total sources.
- **Breezy Onsite Leak Closed & Gate Eligibility Restored (Commit `a831b41`)**:
  - Fixed Breezy parser `is_remote` detection, hoisted onsite regex to Step 0 in `geoGate.ts`, restored `phEligibility` assignment in `recoverGateEligiblePending`.
  - Applied Migration 0046: 22 onsite Sourcefit jobs deactivated, verified remote rows upgraded.
- **PR #150 Merged and Deployed**:
  - Shadow verdict 1.1.0 provenance fields (findings #1, #3, #4) and daily versioned economics snapshot pipeline deployed to Pages and Worker (`9f2871a`).
  - First production snapshot generated by APEX economics clock (`docs/economics-snapshots/snapshots/2026-09-25T08-08-32-705Z.json`): 120 qualified new / 7d = 17.14/day; 867 qualified active / 1,108 all active.
- **Canary -> Active Graduation of 5 Philippine VA Agencies Live**:
  - Migration 0044 (`packages/db/migrations/0044_canary_to_active_graduation.sql`) graduated 5 Breezy sources: `breezy:20four7va`, `breezy:sourcefit`, `breezy:remote-craft`, `breezy:value-virtual-assistants`, `breezy:yokly` to `operational_state = 'active'`.
  - Quarantined: `teamtailor:career.teamtailor.com` (HTTP 404).
