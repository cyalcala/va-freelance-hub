...
| STATUS | TERMINAL — KEEP (code slice `df35fdf`/`848abbe`/`6e31cd7f`, CI/deploy `32605834663` applied migration 0033; owner approved the six recorded candidate rows 2026-08-23; fresh report byte-identical sha256 `86d3a000…` proved zero drift; CAS apply executed 6 planned / 0 skipped, `changes=1` per row; post-state SELECT + reconciliation exact (with_website 344→338, shared-host −2, mismatch −6); undo artifact retained; route smoke passed for all repaired + control rows; focused tests 27 pass, typecheck exit 0). Evidence: `docs/gauntlet/evidence/DATA-05B-directory-website-provenance.md` |
...
| STATUS | CANARY IMPLEMENTED LOCALLY — CRITIC SHIP, NOT DEPLOYED (re-review 2026-08-28 after COMP-01C/D KEEP. Effective fetching set is exactly six `allowed` static sources; mature ~58h30m window has 543/543 allowed real fetches, zero disallowed, unknown/null, or would-block. Isolated `codex/comp01b-canary` implements only the typed `we-work-remotely` canary with default observe elsewhere, explicit enforce-mode failure skips, full provenance, hardened anti-global/dynamic/spread/expansion guard, and tested exact empty-set rollback. Focused 20/0/68; full G3 657/0/1,662 plus typecheck, guardrails, build; fresh independent critic SHIP. Prepared rollback SHA, exact-SHA deploy, and event monitoring remain. Operational rollback proof remains required before terminal acceptance or expansion. Evidence: `docs/gauntlet/evidence/COMP-01B-observation-window-20260824.md`) |
