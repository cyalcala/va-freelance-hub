...
| 3 | AI-01 active model ladder | Evidence-gated defer | REVISE | Current Cloudflare primary sources confirm the three old fallback IDs were deprecated on 2026-05-30, while `llama-3.1-8b-instruct-fast` and `llama-3.3-70b-instruct-fp8-fast` remain active and support JSON mode. Runtime/workflow corrections and conformance tests are implemented on `codex/apex-gauntlet`, but are not approved for production. Isolated run `31691410404` reached the protected Pages preview and archived a provider response showing the account's daily 10,000-neuron allocation was exhausted before the first case. Owner: repository maintainer. Re-run after the quota resets on 2026-08-14 UTC; require corpus quality plus account-neuron evidence before KEEP. |
...
### C-04 - The free neuron budget cannot currently support the planned live acceptance run

- **False assumption:** AI-01's live corpus gate assumed enough Workers AI allocation remained to execute both active models and measure their account-level cost.
- **Contradicting evidence:** authenticated preview run `31691410404` failed on the first 70B fixture with provider code `4006`: the daily free allocation of 10,000 neurons was already exhausted. The archived `workers-ai-geo-evaluation` artifact contains zero model results, so no quality or comparative cost conclusion is valid.
- **Affected plan sections:** AI-01's staging/manual-pulse quality gate, dashboard neuron-delta acceptance, production rollout decision, and the implied cost baseline.
- **Boundary:** this is an external resource gate, not authorization to buy a plan, reduce the corpus after seeing results, switch providers, or infer quality from mocked conformance tests.
- **Smallest alternatives:** (A) re-run the unchanged frozen corpus after the daily allocation resets and archive results plus dashboard evidence; (B) explicitly defer AI-01 with an owner and review date. Unit 3 takes B now: owner repository maintainer, review/re-run 2026-08-14 UTC.
