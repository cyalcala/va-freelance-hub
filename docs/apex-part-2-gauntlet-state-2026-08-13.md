...
| 8 | REL-01 / OPS-03 sole clock and watchdog | Production accepted | REVISE -> KEEP | Test-first slice makes missing `PROXY_SECRET` a Worker failure, verifies the secret before deploy, adds Worker typecheck/dry-run to project CI, and evaluates the durable heartbeat with a bounded post-deploy grace. Critic caught and revision fixed a null/invalid-age false-green. PR #65 merged as `7943cb1`; Worker run `31698037062` proved secret presence, typecheck/dry-run, and deployment; release run `31698036919` passed migrations, FTS, and Pages. Synthetic watchdog run `31698139824` delivered issue #66 at `2026-08-13T12:01:54Z` and verification-closed it at `12:01:56Z`. Final critic: KEEP. Evidence establishes GitHub issue delivery only; it does not claim email/push receipt or a sub-three-hour SLA. |
| 9 | REL-03 invalid URL diagnostics | Code accepted; production evidence pending | REVISE -> KEEP (code) | Final URL normalization now returns `droppedNoUrl`; the all-invalid early outcome uses one builder for its HTTP body and durable diagnostic input; every post-normalization run exit persists the counter. The Worker checks unresolved counters before accepting only the exact `run-lock-held` no-op and rejects `{}`. Critic caught an arbitrary/degraded `skipped: true` false-green and insufficient early-exit wiring coverage; both were revised. 361 tests, app/Worker typechecks, build, guardrails, and Worker dry-run passed. |
