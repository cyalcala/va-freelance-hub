...
STATUS_LOG="OK"
if [ "$STALE" = "1" ] && [ "$RESULT" = "COMPLETED" ]; then STATUS_LOG="STALE_FIXED"; fi
if [ "$STALE" = "1" ] && [ "$RESULT" != "COMPLETED" ]; then STATUS_LOG="STALE_PERSISTS"; fi
if [ "$TOTAL" -lt 273 ]; then STATUS_LOG="WARN: active=$TOTAL"; fi
if awk -v s="$STALENESS" 'BEGIN{exit !(s >= 1)}'; then STATUS_LOG="WARN: stale=${STALENESS}hrs"; fi
if echo "$FEED" | grep -q "No matching signals found"; then STATUS_LOG="WARN: empty signal"; fi
