...
STALENESS=$(echo "$HEALTH" | jq '.vitals.dbStalenessHrs // .vitals.stalenessHrs // 999')
...
if [ "$STALE" = "1" ]; then
  ...
  if [ -z "$TRIGGER_SECRET_KEY" ]; then
    echo "[WARDEN] FAIL: TRIGGER_SECRET_KEY is missing."
    RESULT="FAILED"
  else
...
STATUS_LOG="OK"
if [ "$TOTAL" -lt 273 ]; then
  STATUS_LOG="WARN: active listings < 273"
elif [ "$STALE" = "1" ] && awk "BEGIN{exit !($STALENESS > 1)}"; then
...
if [ -z "$TRIGGER_SECRET_KEY" ]; then
  echo "🚨 NEEDS HUMAN — Missing environment variables (TRIGGER_SECRET_KEY). Cannot trigger harvest from terminal." >> docs/warden-log.md
fi
