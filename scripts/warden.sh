...
# Handle empty/non-JSON response gracefully
...
mkdir -p docs
LOG_STATUS="OK"
if [ "$TOTAL" -lt 273 ] || ! awk "BEGIN{exit !(${STALENESS} < 1)}" || echo "$FEED" | grep -q "No matching signals found"; then
  LOG_STATUS="WARN: check output"
fi
