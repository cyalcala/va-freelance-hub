...
STALENESS=$(echo "$HEALTH" | jq '.vitals.ingestionStalenessHrs // 999' 2>/dev/null)
[ -z "$STALENESS" ] && STALENESS=999
TOTAL=$(echo "$HEALTH" | jq '.vitals.totalActive // 0' 2>/dev/null)
[ -z "$TOTAL" ] && TOTAL=0
...
FINAL_STATUS="OK"
if [ "$TOTAL" -lt 273 ]; then FINAL_STATUS="WARN: low active count"; fi
if ! awk "BEGIN{exit !($STALENESS < 1)}"; then FINAL_STATUS="WARN: high staleness"; fi
if echo "$FEED" | grep -q "No matching signals found"; then FINAL_STATUS="WARN: empty feed"; fi
if [ "$STALE" = "1" ] && awk "BEGIN{exit !($STALENESS < 1)}"; then FINAL_STATUS="STALE_FIXED"; fi
if [ "$STALE" = "1" ] && ! awk "BEGIN{exit !($STALENESS < 1)}"; then FINAL_STATUS="STALE_PERSISTS"; fi

# 4. LOG THE CYCLE RESULT
mkdir -p docs
echo "$(date -u +%Y-%m-%dT%H:%M:%SZ) | stale=${STALENESS}hrs | active=${TOTAL} | status=${FINAL_STATUS}" >> docs/warden-log.md

# 6. EXIT
echo "[WARDEN] cycle done — $(date -u) | stale=${STALENESS}hrs | active=${TOTAL} | $FINAL_STATUS"
