 ...
 *   - greenhouse:ghost (cap: 2)
 *   - greenhouse:nearform (cap: 2)
 *   - breezy:time-etc (cap: 1)
 *
...
const SOURCES: SourceToPromote[] = [
  { sourceId: "greenhouse:ghost", cap: 2 },
  { sourceId: "greenhouse:nearform", cap: 2 },
  { sourceId: "breezy:time-etc", cap: 1 },
];
...
function executeD1SqlFile(sql: string): string {
  const tmpFile = join(process.cwd(), `scripts/graduation/tmp_promote_${Date.now()}_${Math.random().toString(36).slice(2)}.sql`);
...
async function main() {
  console.log("================================================================================");
  console.log("VA FREELANCE HUB — SHADOW -> CANARY PROMOTION EXECUTION");
  console.log("================================================================================");
