 ...
 *   - greenhouse:remotecom (cap: 2)
 *   - greenhouse:wikimedia (cap: 2)
 *
 ...
 *
 * Unit: EX-CANARY-PROMOTION-REMOTECOM-WIKIMEDIA
 * Predecessor pattern: scripts/graduation/promote-gitlab-grafana-canary.ts
 */
...
const SOURCES: SourceToPromote[] = [
  { sourceId: "greenhouse:remotecom", cap: 2 },
  { sourceId: "greenhouse:wikimedia", cap: 2 },
];
...
function queryD1(sql: string): any[] {
  ...
  const cmd = `bun node_modules/wrangler/bin/wrangler.js d1 execute DB --remote --env production --config apps/web/wrangler.jsonc --command "${singleLineSql}" --json`;
...
function executeD1SqlFile(sql: string): string {
  const tmpFile = join(process.cwd(), `scripts/graduation/tmp_promote_remotecom_wikimedia_${Date.now()}_${Math.random().toString(36).slice(2)}.sql`);
  writeFileSync(tmpFile, sql, "utf-8");
  try {
    const cmd = `bun node_modules/wrangler/bin/wrangler.js d1 execute DB --remote --env production --config apps/web/wrangler.jsonc --file="${tmpFile}" --json`;
    return runCmd(cmd);
  } finally {
...
async function main() {
  console.log("================================================================================");
  console.log("VA FREELANCE HUB — GREENHOUSE (REMOTECOM & WIKIMEDIA) CANARY PROMOTION");
  console.log("================================================================================");
